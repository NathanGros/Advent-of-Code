let grow_string str =
	String.concat "?" [str;str;str;str;str]
;;


let grow_list l =
	List.concat [l;l;l;l;l]
;;
...
let parse l = (* (string * int list) array *)
	...
	let tab3 = Array.init (List.length l - 1) (fun i -> (tab.(i).(0), List.init (Array.length (snd tab2.(i))) (fun j -> int_of_string (snd tab2.(i)).(j) ))) in
	Array.init (Array.length tab3) (fun i -> (grow_string (fst tab3.(i)), grow_list (snd tab3.(i))))
;;
...
let rec count_patterns input_str str l nb_cases =
	match l with
	|[] ->
