...
let build_adj_list mat x y =
	...
	if mat.(x+1).(y) = 'v' then l := (follow_path mat (x+2) y S 2) :: (!l);
...
let rec longest tab (x, y) =
	...
	if l = [] then 0
		...
		max_list (List.map (fun (node, n) -> n + longest tab node) l)
;;
...
let _ =
	...
	let result = longest mat_adj (0, 1) in
	print_string "\nThe longest path is ";
	print_int result;
	print_string " long\n"
;;
